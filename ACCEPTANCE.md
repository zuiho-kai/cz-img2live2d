...
日期：2026-09-10 至 2026-09-11（香港时间）。当前结论：三方适配、图层中转、原生编辑入口与导出已经可运行。用户已指出当前头发和整体视觉质量不合格；此前加载、口型和停止检查仅覆盖功能，不构成角色成品验收。

## v0.1.1：头发定向修复

此次复核先复现耸肩牵动长发和发梢撞限位，再修改模型及驱动。它修复具体缺陷，不表示用户已接受整体动作质量。

- 左右外侧长发从身体贴图分成两个独立网格，13 个网格、20 个参数、两张贴图；MOC3 为 8,455,040 字节。原画像素保留，内侧边缘固定，避免露出未绘制的背面。
- 头部参数 ±30 对应 ±20°，身体参数 ±30 对应 ±8°。物理驱动现在使用实际角度，袖子动作也限定在衣服区域。
- 实际 Cubism 顶点检查：耸肩对两块长发的位移为 0；发梢驱动对其他网格的位移为 0；连续重复更新 200 次无累计漂移。
- 30 秒同取景、同动作、同录音 PCM 输入的前后对照：旧版在 901 次采样中有 236 次触及 ±38 像素驱动上限；修正版为 0，发梢驱动峰值约 26.8 像素。嘴部实际图层张开、停止归零；无 WebGL 上下文丢失或页面异常。
- 已检查中性、左右倾斜、耸肩和连续排练的关键画面。仍是正面参考模型，没有逐束头发绑定或大角度转头；不能以这些检查替代整体视觉判断。

[README 内嵌演示与前后对照](README.md#先看现在的小征) · [默认模型](examples/default-live2d/README.md)。以下内容保留 v0.1.0 原始实测范围。
